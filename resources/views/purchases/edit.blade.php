<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900" x-data="purchaseForm()">
                    <form method="POST" action="{{ route('purchases.update', $purchase) }}" class="space-y-6">
                        ...
                        <!-- Total Price Preview -->
                        <div>
                            <label class="block font-medium text-sm text-gray-700">Total Pengeluaran (Otomatis)</label>
                        ...
                        <div>
                            ...
                            <x-text-input id="supplier" name="supplier" type="text" class="mt-1 block w-full" :value="old('supplier', $purchase->supplier)" />
                        ...
                        <div>
                            ...
                            <x-text-input id="purchase_date" name="purchase_date" type="date" class="mt-1 block w-full" :value="old('purchase_date', $purchase->purchase_date->toDateString())" required />
                        ...
                        <div>
                            ...
                            <textarea id="notes" name="notes" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" rows="3">{{ old('notes', $purchase->notes) }}</textarea>
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <a href="{{ route('purchases.index') }}" class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 rounded-md font-semibold text-xs text-gray-700 uppercase tracking-widest shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-25 transition ease-in-out duration-150">
                                Batal
                            </a>
                        </div>
                    </form>
    ...
    <script>
        function purchaseForm() {
            return {
                productId: '{{ old('product_id', $purchase->product_id ?? '') }}',
                itemName: '{{ old('item_name', $purchase->item_name) }}',
                quantity: {{ old('quantity', $purchase->quantity) }},
                unitName: '{{ old('unit', $purchase->unit) }}',
                pricePerUnit: {{ old('price_per_unit', $purchase->price_per_unit) }},
                totalPrice: 0,
                init() {
                    this.calculateTotal();
                    if(this.productId !== '') {
                ...
                onProductChange() {
                    if (this.productId === '') {
                        this.itemName = '{{ $purchase->item_name }}';
                        this.unitName = '{{ $purchase->unit }}';
                        return;
                    }
